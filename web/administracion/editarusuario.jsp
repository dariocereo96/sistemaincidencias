...
<%
    ...
    if (request.getSession().getAttribute("usuario") == null) {
        ...
        if (request.getParameter("id") != null) {
            usuario = usuarioDAO.obtenerUsuarioPorId(Integer.parseInt(request.getParameter("id")));
        } else {
...
<html>
    <head>
        ...
        <title>Editar usuario | Sistema Gestion Incidencias</title>
        <!-- Bootstrap Styles-->
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default">
                                <div class="panel-body">
                                    <form action="${pageContext.request.contextPath}/UsuarioController?action=editar" method="POST">
                                        <input type="hidden" name="idUsuario" value="<%= usuarioSession != null ? usuario.getId() : "-1"%>"/>
                                        <label for="nombre">Nombre de usuario:</label>
                                        <input type="text" value="<%=usuarioSession != null ? usuario.getNombre() : ""%>" class="form-control" style="margin-bottom: 20px" id="nombre" name="nombre" required>

                                        <label for="correo">Correo Electrónico:</label>
                                        <input type="email" value="<%=usuarioSession != null ? usuario.getCorreoElectronico() : ""%>" class="form-control mb-2" style="margin-bottom: 20px" id="correo" name="correo" required>

                                        <label for="contrasena">Nueva contraseña:</label>
                                        <input type="text" value="" class="form-control" style="margin-bottom: 20px" id="contrasena" name="contrasena">

                                        <label for="rol">Rol:</label>
                                        <select id="rol" name="rol" class="form-control" style="margin-bottom: 20px">
                                            <option value="1" <%= usuario.getRolId() == 1 ? "selected" : ""%>>Usuario</option>
                                            <% if (usuarioSession != null && usuarioSession.getRolId() != 5) {%>
                                            <option value="2" <%= usuario.getRolId() == 2 ? "selected" : ""%>>Técnico</option>
                                            <option value="3" <%= usuario.getRolId() == 3 ? "selected" : ""%>>Administrador</option>
                                            <option value="4" <%= usuario.getRolId() == 4 ? "selected" : ""%>>Superadministrador</option>
                                            <option value="5" <%= usuario.getRolId() == 5 ? "selected" : ""%>>Digitador</option>
                                            <%}%>
                                        </select>
